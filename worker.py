...
async def main():
    async with app:
        ...
        while True:
            ...
            now = time.time()
            if now - last_edit > 5:
                translated_files = len([f for f in os.listdir(out_target) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.webp'))]) if os.path.exists(out_target) else 0
        ...
        # ======== CRASH DETECTOR ========
        if process.returncode != 0 or not os.path.exists(out_target):
            error_msg = f"❌ **Task Crashed!**\nGitHub Runner ran out of memory or failed.\nError Code: `{process.returncode}`"
            await app.edit_message_text(c_id, m_id, error_msg)
            return
        # ================================
